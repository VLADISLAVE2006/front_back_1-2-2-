...
function Login({ setIsAuthenticated, setUser }) {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.error || 'Ошибка входа. Проверьте подключение к серверу.';
    ...
    const setTestCredentials = (role) => {
        if (role === 'admin') {
            ...
            setLoginData({ email: 'seller@military.ru', password: 'seller123' });
        } else if (role === 'user') {
                ...
                {isLogin ? (
                    <div className="auth-form active">
                        ...
                        <div style={{ marginBottom: '20px', textAlign: 'center' }}>
                            <p style={{ color: '#666', marginBottom: '10px' }}>Тестовые аккаунты:</p>
                            <button 
                                type="button"
                            ...
                            <button 
                                type="button"
                                onClick={() => setTestCredentials('seller')}
                                style={{ margin: '5px', padding: '5px 10px', background: '#ffc107', color: '#333', border: 'none', borderRadius: '3px', cursor: 'pointer' }}
                            >
                                Продавец
                            </button>
                            <button 
                                ...
                                style={{ margin: '5px', padding: '5px 10px', background: '#6c757d', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer' }}
                            >
                                Пользователь
                            </button>
                        </div>
