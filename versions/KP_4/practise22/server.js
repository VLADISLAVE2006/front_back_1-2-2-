...
async function startServer() {
    try {
        ...
        app.listen(PORT, () => {
            ...
            console.log(`   Админ:     admin@military.ru / admin123`);
            console.log(`   Продавец:  seller@military.ru / seller123`);
            console.log(`   Пользователь: user@example.com / user123`);
            console.log(`\n🖼️  Загруженные изображения сохраняются в папку server/uploads/`);
            console.log(`\n⚡ Redis кэширование активно:`);
            console.log(`   /api/users → кэш 1 минута`);
            console.log(`   /api/users/:id → кэш 1 минута`);
            console.log(`   /api/products → кэш 10 минут`);
            console.log(`   /api/products/:id → кэш 10 минут`);
        });
    } catch (err) {
        console.error('❌ Ошибка подключения к Redis:', err.message);
        console.log('⚠️ Сервер запускается без Redis (кэширование не работает)');

        app.listen(PORT, () => {
            console.log(`\n✅ Сервер запущен на http://localhost:${PORT} (без Redis)`);
        });
    }
}
